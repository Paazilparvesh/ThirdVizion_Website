...
// === IMPORT IMAGES ===
import bullishMarket from "/src/assets/WebStackImages/bullish-market.png";
import cancelLock from "/src/assets/WebStackImages/cancel-lock.png";
import clock3d from "/src/assets/WebStackImages/clock-3d.png";
import communication from "/src/assets/WebStackImages/comunication.png";
import target3d from "/src/assets/WebStackImages/target-3d.png";


// === UPDATED FEATURES LIST (icon → img) ===
const features = [
  {
    img: communication,
    title: "Transparent Communication",
...
// === UPDATED VisualElement (REPLACING ICON WITH IMAGE) ===
const VisualElement = ({ img, isReversed, index }) => {
  const iconRef = useRef(null);
  const containerRef = useRef(null);
    ...
    <div className={`order-1 lg:order-2 ${isReversed ? "lg:col-start-1 lg:row-start-1" : ""}`}>
      <div className={`w-full flex ${isReversed ? "lg:justify-start" : "lg:justify-end"} justify-center`}>
        ...
        >
          ...
          <div ref={iconRef}>
            <img src={img} alt="feature-icon" className="w-24 h-24 object-contain relative z-10" />
          </div>
        </div>
      </div>
    </div>
...
// === MAIN APP (unchanged except img prop added) ===
const App = () => {
  const headerRef = useRef(null);
  const titleRef = useRef(null);
  const descriptionRef = useRef(null);
          ...
          {features.map((feature, index) => {
            ...
            return (
              ...
              >
                ...
                {/* === UPDATED: Pass img instead of icon === */}
                <VisualElement img={feature.img} isReversed={isReversed} index={index} />
              </motion.div>
            );
          })}
        </div>
